# CHAKRAVYUH — full technical documentation

*Short version for judges: see the [README](../README.md). This page has every detail.*
...
| --- | --- | --- |
| 🏹 **DRONA** (gpt-4o) — he designed the chakravyuh | Lead investigator | Triage order of the whole queue · which evidence to pull for each item (6 read-only evidence tools) · when a case is ready to write · whether to send a report back · what to do with weak signals that are *not* cases · a recommendation for the officer |
...
| DRONA: items investigated / tool calls he chose | 13 / 100 |
...
![CHAKRAVYUH architecture](architecture.png)
...
- Entity resolution resolves 2,500 accounts to 1,828 people against a true 1,800: where the only link between two records is a similar name, the resolver refuses to merge them. We accept a few missed merges to guarantee zero false ones.
